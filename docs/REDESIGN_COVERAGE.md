...
| BVX-012 | Dashboard create/edit/save/cancel/delete, rows, panel selector, widgets | `src/views/Dashboard`, `src/stores/dashboard.js` | `DESIGN_NEEDED` | Persisted dashboard and resize behavior, embedded-panel check |
| BVX-013 | Favorite Friends two-pane groups, local/remote state, bulk actions | `src/views/Favorites/FavoritesFriend.vue` | `DESIGN_NEEDED` | Group/copy/move/import/export/action review |
| BVX-014 | Favorite Worlds virtualization, search/sort, instance/favorite actions | `src/views/Favorites/FavoritesWorld.vue` | `DESIGN_NEEDED` | Large-list and action checks |
| BVX-015 | Favorite Avatars, history, apply, invalid-avatar progress | `src/views/Favorites/FavoritesAvatar.vue` | `DESIGN_NEEDED` | Progress/cancel/cache semantics review |
...
| BVX-020 | My Avatars grid/table, filters, editing, tags, upload/crop | `src/views/MyAvatars` | `DESIGN_NEEDED` | Grid/table and editor action review |
| BVX-021 | Instance Activity dates, filters, details, clickable axes/users | `src/views/Charts/components/InstanceActivity.vue` | `DESIGN_NEEDED` | Data processor/worker unchanged, chart interaction check |
| BVX-022 | Mutual Friends worker graph, privacy, cancel/progress, context menu | `src/views/Charts/components/MutualFriends.vue` | `DESIGN_NEEDED` | Worker lifecycle and graph action check |
| BVX-023 | Hot Worlds rankings, windows, trends, detail sheet | `src/views/Charts/components/HotWorlds.vue` | `DESIGN_NEEDED` | Ranking/filter and entity click-through check |
| BVX-024 | Tools catalog, categories, pinning, route/dialog/native actions | `src/views/Tools`, `src/composables/useToolActions.js` | `DESIGN_NEEDED` | Pin persistence, native action/toast check |
| BVX-025 | Gallery tabs, upload/crop/delete, inventory/redeem/consume | `src/views/Tools/Gallery.vue` | `DESIGN_NEEDED` | Tab and destructive/upload workflow check |
| BVX-026 | Screenshot Metadata search, table/list, preview, file/clipboard/upload | `src/views/Tools/ScreenshotMetadata.vue` | `DESIGN_NEEDED` | Metadata parser/action and keyboard navigation check |
| BVX-027 | Settings IA and System/Interface/Social tabs | `src/views/Settings/components/Tabs` | `DESIGN_NEEDED` | Store side effects, platform gates, persistence check |
| BVX-028 | Settings Notifications/VR/Media/Integrations tabs | `src/views/Settings/components/Tabs` | `DESIGN_NEEDED` | Immediate controls, dialogs, native/VR dependencies |
| BVX-029 | Settings Advanced diagnostics, cache, DB, security, destructive actions | `src/views/Settings/components/Tabs/AdvancedTab.vue` | `DESIGN_NEEDED` | Warning/confirm/restart semantics and action review |
